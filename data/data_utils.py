...
def triangluation_bpa(pnts, test_pnts=None, full_comb=False, perturb=0):
    ...
    if perturb > 0:
        pnts_r = pnts[:, :3] + np.random.randn(*pnts.shape) * np.array(distances)[...,None] * perturb
        pcd.points = o3d.utility.Vector3dVector(pnts_r[:, :3])
        pcd.normals = o3d.utility.Vector3dVector(pnts_r[:, :3] / np.linalg.norm(pnts_r[:, :3], axis=-1, keepdims=True))
        distances_r = pcd.compute_nearest_neighbor_distance()
        avg_dist_r = np.mean(distances_r)
        radius = 2 * avg_dist_r
