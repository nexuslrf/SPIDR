...
class ViewMLP(PointAggregator):
    ...
    def viewmlp_init(self, opt, block_init_lst):
        ...
        in_channels = opt.point_features_dim + (0 if opt.agg_feat_xyz_mode == "None" else self.pnt_channels) \
            - (opt.weight_feat_dim if opt.agg_distance_kernel in ["feat_intrp", "meta_intrp"] else 0) \
            - (opt.sh_degree ** 2 if opt.agg_distance_kernel == "sh_intrp" else 0) - (7 if opt.agg_distance_kernel == "gau_intrp" else 0)
        ...
        if opt.diffuse_branch_channel > 0:
            diffuse_block = []      
            in_channels = opt.shading_feature_num + (3 if opt.diffuse_branch_color_in else 0)
            out_channels = int(opt.shading_feature_num / 2)
        ...
        for i in range(opt.shading_color_mlp_layer - 1):
            ...
            in_channels = out_channels
        color_block.append(nn.Linear(in_channels, color_channels))
    ...
    def viewmlp(self, s_pts, sample_loc, sample_loc_w, sample_ray_dirs, 
        ...
        if self.opt.agg_intrp_order == 0:
            feat = torch.sum(s_pts['embedding'] * weight[..., None], dim=-2)
            ...
            Rdeform = s_pts['Rdeform'] if s_pts['Rdeform'] is not None else None
            ...
            if Rdeform is not None:
                dists_flat[..., :3] = (Rdeform * dists_flat[..., :3, None]).sum(-2) # R.T @ dist
            ...
            if self.opt.attr_weight_mode == 'agg':
                attr_weight_valid = weight[ray_valid].detach()
        ...
        if self.opt.shading_feature_mlp_layer3>0:
            ...
            if "1" in list(self.opt.point_dir_mode) and s_pts['dir'] is not None:
                if "2" in list(self.opt.point_dir_mode):
                    sampled_dir = s_pts['normal'].view(-1, s_pts['normal'].shape[-1]).detach()
                else:
        ...
        agg_gradient, pnt_feat_var, roughness_var = None, None, None
        # if weight > 0?
        ...
        if self.opt.agg_intrp_order == 1:
            ...
            alpha = self.raw2out_density(alpha_out[...,:1])
            ...
            if self.opt.apply_pnt_mask > 0:
                ...
                alpha_holder = alpha
            alpha = alpha_holder.view(B * R * SR, K, alpha_holder.shape[-1])[ray_valid]
            alpha = torch.sum(alpha * weight_valid, dim=-2) # alpha:
            ...
            if self.opt.depth_only:
                return alpha_placeholder, {'density': alpha}
            
            if (self.opt.learn_point_normal and self.training) or self.opt.return_agg_normal:
                if alpha.requires_grad:
                    loc_gradient = -torch.autograd.grad(alpha, sample_loc_w, torch.ones_like(alpha), 
            ...
            with torch.set_grad_enabled(self.training or self.opt.brdf_training):
                ...
                if self.opt.learn_point_normal:
                    dir_valid = s_pts['normal'].reshape(-1, K, 3)[ray_valid]
            ...
            if sample_ray_dirs is None:
                others = {'density': alpha, 'specular': specular,
                        'roughness': roughness, 'pred_gradient': pred_gradient,
                        'pnt_diffuse_var': pnt_diffuse_var, 'pnt_normal_var': pnt_normal_var,
                        'pnt_roughness_var': roughness_var, 'diffuse_color': diffuse_color, 'point_color': point_color
                        }
                others['gradient'] = pred_gradient if self.opt.learn_point_normal else agg_gradient
        ...
        others = {'agg_gradient': agg_gradient, 'density': alpha, 
                  'diffuse_color': diffuse_color, 'pred_gradient': pred_gradient,
                  'specular': specular, 'roughness': roughness, 
                  'pnt_diffuse_var': pnt_diffuse_var, 'pnt_normal_var': pnt_normal_var,
                  'pnt_feat_var': pnt_feat_var, 'pnt_roughness_var': roughness_var,
                  'point_color': point_color
                } # 'xyz_gradient': pts_gradient, 
        if self.opt.orientation_weight > 0:
            others['viewdirs'] = viewdirs
        # if self.opt.learn_diffuse_color:
        #     others['diffuse_color'] = diffuse_color
        # if self.opt.learn_point_normal:
        #     others['pred_gradient'] = pred_gradient
