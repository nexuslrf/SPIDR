...
def get_scheduler(optimizer, opt):
    step = opt.sched_steps if hasattr(opt, 'sched_steps') else 0
    if opt.lr_policy == 'lambda':
        def lambda_rule(it):
            lr_l = 1.0 - max(0, it - opt.niter) / float(opt.niter_decay + 1)
            return lr_l

        scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda_rule, last_epoch=step-1)
    elif opt.lr_policy == 'step':
        scheduler = lr_scheduler.StepLR(optimizer, step_size=opt.lr_decay_iters, gamma=0.1, last_epoch=step-1)
    elif opt.lr_policy == 'plateau':
        scheduler = lr_scheduler.ReduceLROnPlateau(optimizer,
                                                   mode='min',
                                                   factor=0.2,
                                                   threshold=0.01,
                                                   patience=5, last_epoch=step-1)
...
def init_seq(s, init_type='xavier_uniform'):
    ...
    for a, b in zip(s[:-1], s[1:]):
        if isinstance(b, nn.ReLU) or isinstance(b, nn.Softplus):
...
def positional_encoding(positions, freqs, ori=False):
    ...
    # Attention!!!!
    # cat & stack have completely different coordinate order! 
    # cat: [B, C, D], stack: [B, D, C]. C is sin/cos func.
    return pts